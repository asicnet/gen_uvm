...
#-----------------------#-----------------------#-----------------------#-----------------------#-----------------------
class UVM_CREATE_ENV(UVM_BASE):
    '''Class for UVM Generator
    ...
    def check_common_pkg (self):
      ...
      if fname.exists():
        tb['common_pkg']['name'] = self.get_pkg_name(fname)
      else:
        tb['common_pkg']['name'] = re.sub(r'\..*','',fname.name)
        warning_prompt("common_pkg file ",str(fname.name)," not found",doit=True)
    ...
    def check_common_env_pkg(self):
      ...
      if ( fname.exists() ):
        tb['common_env_pkg']['name'] = self.get_pkg_name(fname)
      else:
    ...
    def after_parse_and_checks (self):
    ...
    def create_directories_and_copy_files (self):
