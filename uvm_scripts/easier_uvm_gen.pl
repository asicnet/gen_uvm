...
## 05/04/2017  Ng Mow Song, MRSemi  - copy and generate files.f for .v file 
## 20/04/2017  Ng Mow Song, MRSemi  - Fixed the agent sequence item name and its file name
...
sub parse_template {
    ...
    for ( ; ; ) {
        ...
        if ( $param_name =~ /trans_item/i ) {
            $agent_item = $agent_name . "_" . $param_value;
