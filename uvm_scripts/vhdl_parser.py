...
class VhdlObject(object):
  ...
  def __str__(self):
    return '{} : {} {}'.format(self.name, self.kind, self.desc)
...
class VhdlEntity(VhdlObject):
  ...
  Args:
    ...
    package (str): Package containing the entity
    ...
    if action == 'section_meta':
      ...
      if groups[0]:  
        for i in range(1,items+1):
          temp = groups[0]
          for g in generics:
            temp = re.sub(rf'\b{g.name}\b',g.name,temp,flags=re.IGNORECASE)
          ports[-i].default_value = temp
      
    elif action == 'generic_param_default':
      if groups[0]:  
        for i in range(1,items+1):
          generics[-i].default_value = groups[0]
      ...
      res = re.search('\((.*)\)',arange)
      if res:
        new_range =  re.sub('\s*(downto|to)\s*',' : ',res.groups()[0]).strip()
        new_range =  re.sub(r'\(\s*','(',new_range)
        new_range =  re.sub(r'\s*\)',')',new_range)
        for i in generics:
          new_range = re.sub(rf'\b{i.name}\b',i.name,new_range,flags=re.IGNORECASE)

      items=0
      for i in param_items:
        items+=1
        ports.append(VhdlParameter(i, mode, ptype + arange,'',''))
        ports[-1].range = '['+new_range+']'
      param_items = []
      last_item = ports[-1]
      ...
      vobj = VhdlEntity(name, cur_package, ports, generics, dict(sections), metacomments)
      objects.append(vobj)
      last_item = None
      metacomments = []
      ...
      if groups[0]:  
        for i in range(1,items+1):
          if not generics[-i].desc:
            generics[-i].desc = re.sub('\s+',' ',groups[0]) 

    elif action == 'comment':
      if groups[0]:  
        for i in range(1,items+1):
          if not ports[-i].desc:
            ports[-i].desc = re.sub('\s+',' ',groups[0])
